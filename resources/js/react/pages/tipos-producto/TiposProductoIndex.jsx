...
export default function TiposProductoIndex() {
    ...
    const load = async () => {
        setLoading(true);
        const { data } = await axios.get("/api/admin/tipos-producto");
        setItems(data);
        setLoading(false);
    };
    ...
    useEffect(() => {
        load().catch(() => {
            toast.error("No se pudo cargar tipos de producto");
            setLoading(false);
        });
    }, []);
    ...
    const confirmDelete = async () => {
        ...
        try {
            await axios.delete(`/api/admin/tipos-producto/${deleteItem.id}`);
                ...
                "
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">
                                Configura cómo se estructuran los productos
                            </p>
                        </div>
                    </div>
                    ...
                    "
                        >
                            <i className="mgc_ruler_line text-lg"></i>
                            Tamaños
                        </a>
    ...
    "
                    ...
                    <table className="w-full text-sm">
                        ...
                        <thead className="bg-slate-50 border-b border-gray-200">
                            <tr className="text-left text-xs font-semibold tracking-wider text-gray-600 uppercase">
                                ...
                                <th className="px-7 py-4 w-[180px]">
                                    Usa tamaños
                                </th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loading ? (
                                ...
                                items.map((row) => (
                                    <tr
                                        key={row.id}
                                        className="group hover:bg-cyan-50/40 transition"
                                    >
                                        {/* ID */}
            ...
            {deleteItem && (
                <ConfirmDeleteModal
                    ...
                    message={`¿Seguro que deseas eliminar "${deleteItem.nombre}"? Esta acción no se puede deshacer.`}
